...
define ( 'WUA_THEME_URL', get_template_directory_uri() );
...
if ( ! function_exists( 'wua_theme_setup' ) ) {
	function wua_theme_setup() {
		...
		function app_enqueue_assets() {
			$manifest = app_get_manifest();

			# Enqueue Custom JS files
			if ( ! empty( $manifest['index.html']['file'] ) ) {
				wp_enqueue_script(
					'theme-js-bundle',
					WUA_THEME_URL . '/dist/' . $manifest['index.html']['file'],
					array( 'jquery' ), // deps
			...
			if ( ! empty( $manifest['index.html']['css'][0] ) ) {
				wp_enqueue_style(
					'theme-css-bundle',
					WUA_THEME_URL . '/dist/' . $manifest['index.html']['css'][0],
				);
			}
		}
		...
		include_once( WUA_INC_DIR . 'asset-build-process.php' );
